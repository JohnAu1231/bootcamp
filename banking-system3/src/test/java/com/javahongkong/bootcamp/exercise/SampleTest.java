	...
	public void threadTest() throws Exception {
		...
		Runnable open = () -> {
			for (int i = 0; i < 1000; i++) {
				...
				bank2.openConsumerAccount(new Person("A","B",2+i),1111,0.0);
			
			}
		};
	
		Thread thread1 = new Thread(open);
		Thread thread2 = new Thread(open);
		thread1.start();
		thread2.start();
		try {
			thread1.join();
		...
		Assert.assertEquals(bank2.getCounter(), 2000L, 0);
